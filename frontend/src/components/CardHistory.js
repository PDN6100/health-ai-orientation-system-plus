...
export default function CardHistory({ layout }) {
  ...
  const diseaseListCard = currentData.map((element, index) => (
    ...
    >
      ...
      >
        <Calendar
          style={{ height: '18px', color: 'green', fontWeight: '900', marginRight: '3px' }}
        />{' '}
        {element.createdAt}
      </div>
      <div className="disease" style={{ fontSize: '20px', fontWeight: '600' }}>
        {element.disease}
      </div>
      <h5
        style={{
      ...
      <div className="symptoms" style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
        {' '}
        {element.Symptomes.map((s, i) => (
          <div
            key={i}
      ...
      <div
        ...
        style={{ display: 'flex', gap: '20px', alignItems: 'center', justifyContent: 'start' }}
      >
        <div
          style={{
            position: 'relative',
  ...
  return layout === 'list' ? (
    <ThemeProvider theme={theme}>
      ...
      <div className="responsive-list" style={{ display: 'none' }}>
        {responsiveList}
      </div>
    </ThemeProvider>
  ) : (
